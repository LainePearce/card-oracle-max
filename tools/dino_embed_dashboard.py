#!/usr/bin/env python3
"""
DINOv2 Embed Dashboard — 2026 vectorization status (GPU worker-0, port 8080).

Tracks the dino-embed fleet: per-day embed status, vectors written vs OS docs,
and which GPU workers are running the dino-embed service. Sibling of the image
-archive dashboard, which now lives on the CPU worker-0.

    python tools/dino_embed_dashboard.py [--port 8080] [--once]
"""
...
from tools.dino_embed_common import (
    s3_client, QUEUE_BUCKET, QUEUE, ACTIVE, COMPLETE, list_dates,
)
...
def poll_s3_state() -> dict:
    ...
    if complete:
        with ThreadPoolExecutor(max_workers=16) as ex:
            for d, m in ex.map(lambda d: _read(s3, COMPLETE, d), complete):
                markers[d] = m
    active_jobs = []
    if active:
        with ThreadPoolExecutor(max_workers=8) as ex:
            for d, m in ex.map(lambda d: _read(s3, ACTIVE, d), active):
                m["date"] = d
    ...
    # with its embed status + counts, for the backfill table.
    ...
    for n in sorted(set(queued + active + complete)):
        ...
        if n in active:
            ...
            nonebay.append({"index": n, "status": "active",
                            "embedded": j.get("vectors", 0), "total": j.get("total", 0),
                            "host": j.get("host", ""),
                            "updated_at": j.get("updated_at") or j.get("claimed_at")})
        elif n in complete:
            m = markers.get(n, {})
            nonebay.append({"index": n, "status": "complete",
                            "embedded": m.get("embedded", 0), "total": m.get("total", 0)})
        else:
...
def poll_workers():
    ...
    for i in range(N_WORKERS):
        t = threading.Thread(target=lambda i=i: results.__setitem__(i, poll_worker(i, SSH_TARGETS[i])), daemon=True)
...
_state = {"updated_at": None, "next_update_at": None}
_prev = {"complete": None, "t": None}


def _eta(done, remaining):
    now = time.monotonic()
    if _prev["complete"] is not None and _prev["t"] is not None:
        dc, dt = done - _prev["complete"], now - _prev["t"]
        if dt > 0 and dc > 0:
            secs = remaining / (dc / dt); h = secs / 3600
            return f"~{int(secs/60)}m" if h < 1 else f"~{h:.1f}h" if h < 24 else f"~{h/24:.1f}d"
    _prev["complete"], _prev["t"] = done, now
    return None
...
<title>DINOv2 Embed Dashboard</title><style>
...
<header><div><h1>Card Oracle — <span>DINOv2 Embed Dashboard</span></h1>
<div style="color:var(--muted);font-size:11px;margin-top:2px">2026 archived images → DINOv2 1024-d → Qdrant cards_dinov2 · 12 × GPU workers</div></div>
...
<div class="card green"><div class="label">Days Embedded</div><div class="value" id="s-complete">—</div><div class="sub">days vectorized</div></div>
<div class="card yellow"><div class="label">Days Active</div><div class="value" id="s-active">—</div><div class="sub">in progress</div></div>
<div class="card blue"><div class="label">Days Queued</div><div class="value" id="s-queued">—</div><div class="sub">archived, pending embed</div></div>
<div class="card green"><div class="label">Vectors Written</div><div class="value" id="s-vectors">—</div><div class="sub">DINOv2 1024-d</div></div>
<div class="card yellow"><div class="label">ETA</div><div class="value" id="s-eta" style="font-size:18px">—</div><div class="sub">all queued days</div></div>
</div>
<div class="section"><div class="section-title">Active Days — embedding now</div>
<div style="overflow-x:auto"><table class="worker-table"><thead><tr><th>Date</th><th>Worker</th><th>Vectors</th><th>Total</th><th>Progress</th><th>Updated</th></tr></thead>
...
<div class="section-title" style="margin:0">2026 Embed Coverage</div>
<div class="legend"><div class="legend-item"><div class="legend-dot complete"></div>Embedded</div><div class="legend-item"><div class="legend-dot active"></div>Active</div><div class="legend-item"><div class="legend-dot queued"></div>Queued</div><div class="legend-item"><div class="legend-dot not_in_queue"></div>Not queued</div></div></div>
...
<div style="overflow-x:auto"><table class="worker-table"><thead><tr><th>Index</th><th>Status</th><th>Embedded</th><th>Manifest total</th><th>Coverage</th><th>Updated</th></tr></thead>
<tbody id="nonebay"><tr><td colspan="6" class="loading">Loading…</td></tr></tbody></table></div></div>
<div class="section"><div class="section-title">Worker Fleet — dino-embed service</div>
...
function renderSummary(d){
  ...
  document.getElementById('s-vectors').textContent=fmt(d.total_vectors);
...
function renderActive(jobs){const tb=document.getElementById('active-jobs');
  ...
  tb.innerHTML=jobs.map(j=>{const a=j.vectors||0,t=j.total||0;const pct=t?Math.min(100,a/t*100):0;
...
function renderCal(cov,perday){const grid=document.getElementById('calendar');
  ...
  grid.innerHTML=keys.map(k=>{const [y,m]=k.split('-').map(Number);const dm=months[k];
    ...
    const cells=[];for(let day=1;day<=dim;day++){const ds=`${y}-${String(m).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
      ...
      const tip=pd?`${ds} · ${fmt(pd.vectors)}/${fmt(pd.total)}`:`${ds} · ${s}`;
...
function renderNonebay(rows){const tb=document.getElementById('nonebay');
  if(!rows||!rows.length){tb.innerHTML='<tr><td colspan="6" class="svc-unknown" style="text-align:center;padding:14px">No non-eBay indices seeded</td></tr>';return;}
  tb.innerHTML=rows.map(r=>{const a=r.embedded||0,t=r.total||0;const pct=t?Math.min(100,a/t*100):0;
    ...
    return `<tr><td><b>${r.index}</b></td><td>${st}</td><td>${r.status==='queued'?'—':fmt(a)}</td><td>${r.status==='queued'?'—':fmt(t)}</td><td>${cov}</td><td class="ip">${r.updated_at?fmtTime(r.updated_at):'—'}</td></tr>`;
  }).join('');
}
...
def main():
    ...
    args = ap.parse_args()
    print("[dino-dashboard] initial poll…", flush=True)
    poll_and_update()
    if args.once:
        with _state_lock:
    ...
    print(f"[dino-dashboard] serving at http://localhost:{args.port}", flush=True)
    srv.serve_forever()
